...
class MockControlCenterTransport implements ControlCenterHttpTransport {
  MockControlCenterTransport({
    this.postHandler,
    this.getHandler,
  });

  Future<Map<String, dynamic>> Function(String url, Map<String, Object?> payload)? postHandler;
  Future<Map<String, dynamic>> Function(String url)? getHandler;

  final List<Map<String, dynamic>> postedCalls = [];

  @override
  Future<Map<String, dynamic>> postJson(String url, Map<String, Object?> payload) async {
...
void main() {
  ...
  group('PASO 1 & 2: Activación y Validación RS256 de Licencia', () {
    test('activación online guarda token cifrado y metadatos del cliente', () async {
      ...
      final success = await LicenciaService.instance.activarDesdeControlCenter(
    ...
    test('validación al iniciar falla cerrado si la firma RS256 es inválida', () async {
      final hwService = HardwareFingerprintService();
      final fingerprint = await hwService.generateFingerprint();
    ...
    test('modo de gracia respeta la fecha expires_at del token RS256 y vence si pasa el límite', () async {
      final hwService = HardwareFingerprintService();
      final fingerprint = await hwService.generateFingerprint();
      ...
      final license = LicenciaInfo(
        ...
        hardwareFingerprint: fingerprint,
        offlineToken: token,
        lastSuccessfulValidationAt: DateTime.now().toUtc().subtract(const Duration(days: 1)),
      );
      ...
      final validWithinGrace = await LicenciaService.instance.validarConControlCenterOGracia(
      ...
      final validAfterExpiry = await LicenciaService.instance.validarConControlCenterOGracia(
        client: ControlCenterLicenseClient(transport: offlineClient),
        validationService: validator,
        fingerprintService: hwService,
  ...
  group('PASO 4 & 5: Polling de Comandos y Stub de Acceso Remoto', () {
    test('comando de acceso remoto registra stub sin ejecutar captura/streaming y responde ACK', () async {
      final commandData = {
        'id': 'CMD-REMOTE-001',
      ...
      final result = await ControlCenterAgent.processCommandForTests(commandData);
      ...
      expect(notifications.first['titulo'], equals('MERKA solicita acceso remoto'));
      expect(notifications.first['detalle'], contains('Stub pendiente Fase RA'));
    ...
    test('CCCommandsProcessor procesa comandos soportados (bloquear, activar, notificar)', () async {
      final msgCmd = ComandoRemoto(
        id: 'CMD-MSG-01',
        tipo: TipoComando.mensaje_admin,
        parametros: {'titulo': 'Mensaje urgente', 'detalle': 'Actualizar datos'},
        timestamp: DateTime.now(),
      );
      final resMsg = await CCCommandsProcessor.instance.procesarComando(msgCmd);
      expect(resMsg.exito, isTrue);

      final lockCmd = ComandoRemoto(
        id: 'CMD-LOCK-01',
        tipo: TipoComando.bloquear_instalacion,
        parametros: {},
        timestamp: DateTime.now(),
      );
      final resLock = await CCCommandsProcessor.instance.procesarComando(lockCmd);
      expect(resLock.exito, isTrue);

      final isLocked = await CCCommandsProcessor.instance.verificarInstalacionBloqueada();
      expect(isLocked, isTrue);

      final unlockCmd = ComandoRemoto(
        id: 'CMD-UNLOCK-01',
        tipo: TipoComando.activar_instalacion,
        parametros: {},
        timestamp: DateTime.now(),
      );
      final resUnlock = await CCCommandsProcessor.instance.procesarComando(unlockCmd);
      expect(resUnlock.exito, isTrue);

      final isLockedAfterUnlock = await CCCommandsProcessor.instance.verificarInstalacionBloqueada();
