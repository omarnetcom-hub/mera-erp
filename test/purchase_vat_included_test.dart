...
void main() {
  late Database db;
  const cop = Currency(code: 'COP', decimalPlaces: 0);
  ...
  test('TEST 1: Compra con precio_incluye_iva=true a \$119.000 (19%) → base \$100.000, IVA \$19.000', () async {
    ...
    expect(subtotal.minorUnits, equals(100000));
    expect(impuesto.minorUnits, equals(19000));
    expect(totalConIva.minorUnits, equals(119000));

    final result = await useCase.execute(CreatePurchaseRequest(
      supplierId: 1,
    ...
    expect(result.subtotal.minorUnits, equals(100000));
    expect(result.tax.minorUnits, equals(19000));
    expect(result.total.minorUnits, equals(119000));
    ...
    expect(compraRow.first['subtotal'], equals(100000));
    expect(compraRow.first['impuesto_total'], equals(19000));
    expect(compraRow.first['total'], equals(119000));
    ...
    expect(detalleRow.first['subtotal'], equals(100000));
    expect(detalleRow.first['impuesto_total'], equals(19000));
  });

  test('TEST 2: Compra con precio_incluye_iva=false mantiene comportamiento tradicional', () async {
    final useCase = CreatePurchaseUseCase();
    final companyId = await DatabaseHelper.instance.obtenerEmpresaActivaId();
    ...
    expect(subtotal.minorUnits, equals(100000));
    expect(impuesto.minorUnits, equals(19000));
    expect(total.minorUnits, equals(119000));
  ...
  test('TEST 3: F300 reporta correctamente IVA descontable con precio_incluye_iva=true', () async {
    final useCase = CreatePurchaseUseCase();
    final companyId = await DatabaseHelper.instance.obtenerEmpresaActivaId();
    ...
    await useCase.execute(CreatePurchaseRequest(
      supplierId: 1,
      supplierName: 'Proveedor Test',
    ...
    final detalleF300 = await DatabaseHelper.instance.obtenerDetalleFormulario300(
      anio: anio,
      mes: mes,
    );

    final comprasF300 = detalleF300.where((row) => row['origen'] == 'compra').toList();
    ...
    final borradorF300 = await DatabaseHelper.instance.obtenerBorradorFormulario300(
      anio: anio,
      mes: mes,
    );
  ...
  test('TEST 4: Múltiples líneas con diferentes configuraciones de precio_incluye_iva', () async {
    final useCase = CreatePurchaseUseCase();
    final companyId = await DatabaseHelper.instance.obtenerEmpresaActivaId();
