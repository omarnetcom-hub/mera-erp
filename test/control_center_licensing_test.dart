...
void main() {
  ...
  test('activacion online verifica RS256 y guarda licencia cifrada', () async {
    ...
    expect(rows.single['valor'].toString(), contains('AES-256-CBC'));
  });

  test('validacion online invalida bloquea y red caida usa gracia firmada', () async {
    final token = _createToken(keys.privateKey, _validPayload());
    ...
    final invalid = await LicenciaService.instance.validarConControlCenterOGracia(
      currentHardwareFingerprint: 'HW-TEST-001',
      validationService: validator,
      client: ControlCenterLicenseClient(
    ...
    final grace = await LicenciaService.instance.validarConControlCenterOGracia(
      currentHardwareFingerprint: 'HW-TEST-001',
      validationService: validator,
  ...
  test('comando solicitar acceso remoto solo crea stub de consentimiento', () async {
    final result = await ControlCenterAgent.processCommandForTests({
      'id': 'cmd-ra-1',
...
Future<void> _createMinimalSchema(Database db) async {
  await db.execute('CREATE TABLE app_config (clave TEXT PRIMARY KEY, valor TEXT)');
...
class _FakeTransport implements ControlCenterHttpTransport {
  const _FakeTransport({this.post = const {}, this.get = const {}});
  ...
  @override
  Future<Map<String, dynamic>> getJson(String url) async => get;
}
