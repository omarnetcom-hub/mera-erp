...
class _CurrencyConfigPageState extends State<CurrencyConfigPage> {
  ...
  Future<void> _setDefaultCurrency(Currency currency) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${currency.code} establecida como moneda predeterminada')),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error al establecer moneda predeterminada: $e')),
        );
      }
    } finally {
            ...
            if (_defaultCurrency != null)
              ListTile(
                leading: CircleAvatar(
                  child: Text(_defaultCurrency!.symbol),
                ),
                title: Text('${_defaultCurrency!.code} - ${_defaultCurrency!.name}'),
        ...
        children: [
          ...
          ..._currencies.map((currency) => Card(
                ...
                child: ListTile(
                  ...
                  subtitle: Text('Símbolo: ${currency.symbol} - Decimales: ${currency.decimalPlaces}'),
