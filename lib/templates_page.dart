...
class _TemplatesPageState extends State<TemplatesPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final templates = await _templateService.getTemplatesByCompany(db, companyId);
  ...
  Future<void> _setAsDefault(DocumentTemplate template) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${template.name} establecida como predeterminada')),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error al establecer plantilla predeterminada: $e')),
        );
      }
    } finally {
  ...
  Future<void> _duplicateTemplate(DocumentTemplate template) async {
    ...
    if (result == true) {
      ...
      try {
        final db = await DatabaseHelper.instance.database;
        await _templateService.duplicateTemplate(db, template.id!, nameController.text.trim());
        await _loadData();
  ...
  Future<void> _deleteTemplate(DocumentTemplate template) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('¿Está seguro de eliminar la plantilla "${template.name}"?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('Cancelar'),
  ...
  Future<void> _showTemplateDialog(DocumentTemplate? template) async {
    ...
    final htmlController = TextEditingController(text: template?.htmlContent ?? '');
    final typeController = TextEditingController(text: template?.templateType ?? 'invoice');
    ...
    if (result == true) {
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(template == null ? 'Plantilla creada' : 'Plantilla actualizada')),
          );
        }
      } catch (e) {
        if (mounted) {
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              const Text('Variables disponibles:', style: TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 4),
              ...DocumentTemplate.availableVariables.entries.map((entry) => Padding(
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        leading: CircleAvatar(
                          backgroundColor: _getTemplateTypeColor(template.templateType),
                          child: Icon(
