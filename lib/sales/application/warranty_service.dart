...
class WarrantyService {
  ...
  Future<int> generateWarrantyForSale(
    ...
    final endDate = DateTime(startDate.year, startDate.month + durationMonths, startDate.day);
  ...
  Future<List<Warranty>> getWarrantiesByProduct(Database db, int productId) async {
    final maps = await db.query(
      'warranties',
      where: 'product_id = ?',
      whereArgs: [productId],
  ...
  Future<List<Warranty>> getWarrantiesByCustomer(Database db, int customerId) async {
    final maps = await db.query(
      'warranties',
      where: 'customer_id = ?',
  ...
  Future<List<Warranty>> getExpiredWarranties(Database db, int companyId) async {
  ...
  Future<void> updateWarrantyStatus(Database db, int warrantyId, String status) async {
    await db.update(
      'warranties',
      {
        'status': status,
  ...
  Future<List<Map<String, dynamic>>> getWarrantyClaims(Database db, int warrantyId) async {
    final maps = await db.query(
      'warranty_claims',
      where: 'warranty_id = ?',
      whereArgs: [warrantyId],
  ...
  Future<List<Map<String, dynamic>>> getPendingClaims(Database db, int companyId) async {
  ...
  Future<Map<String, dynamic>> getWarrantyStatistics(Database db, int companyId) async {
  ...
  Future<Warranty?> getWarrantyBySaleNumber(Database db, String saleNumber) async {
    final maps = await db.query(
      'warranties',
      where: 'sale_number = ?',
  ...
  Future<void> extendWarranty(Database db, int warrantyId, int additionalMonths) async {
    final warranty = await getWarrantyById(db, warrantyId);
    if (warranty == null) return;
    
    final newEndDate = warranty.endDate.add(Duration(days: 30 * additionalMonths));
