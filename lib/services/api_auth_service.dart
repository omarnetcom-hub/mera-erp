...
class ApiAuthService {
  ...
  Future<ApiKey?> validarApiKey(String key) async {
    ...
    final apiKey = ApiKey.fromMap(rows.first);

    // Actualizar última uso
    await db.update(
      'api_keys',
      {'ultima_uso': DateTime.now().toIso8601String()},
      where: 'id = ?',
      whereArgs: [apiKey.id],
    );
      ...
      return (Request request) async {
        ...
        _rateLimitCache.removeWhere((k, timestamps) {
          timestamps.removeWhere((t) => 
              now.difference(t).inMinutes > apiKey.rateWindowMinutes);
  ...
  Future<void> registrarAcceso(ApiKey apiKey, String endpoint, String metodo) async {
    final db = await DatabaseHelper.instance.database;
