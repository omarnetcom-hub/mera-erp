...
class HybridSyncService {
  ...
  Future<void> initialize() async {
    ...
    // Crear tabla de tracking de cambios locales
    await db.execute('''
    ...
    // Crear tabla de tracking de sincronización
    await db.execute('''
    ...
    await _checkPostgresAvailability();
    
    // Iniciar Sincronización automática si PostgreSQL está disponible
  ...
  Future<void> _checkPostgresAvailability() async {
    try {
      final postgresService = PostgresService();
      await postgresService.connection;
      _isPostgresAvailable = true;
      debugPrint('PostgreSQL is available for sync');
    } catch (e) {
      _isPostgresAvailable = false;
      debugPrint('PostgreSQL is not available: $e');
    }
  }
  ...
  Future<void> _pullRemoteChanges() async {
    ...
    try {
      ...
      try {
        ...
        DateTime? lastSync;
        int? lastSyncRecordId;
        
        if (metadataRows.isNotEmpty) {
          lastSync = DateTime.tryParse(metadataRows.first['last_sync_timestamp'] as String);
          lastSyncRecordId = metadataRows.first['last_sync_record_id'] as int?;
        }
