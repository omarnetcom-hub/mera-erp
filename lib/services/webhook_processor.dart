...
class WebhookProcessor {
  ...
  Future<void> configurarSecret(WebhookSource source, String secret) async {
    _webhookSecrets[source] = secret;

    final db = await DatabaseHelper.instance.database;
    await db.insert(
      'app_config',
      {
        'clave': 'webhook_secret_${source.name}',
        'valor': secret,
      },
  ...
  Future<String?> obtenerSecret(WebhookSource source) async {
    if (_webhookSecrets.containsKey(source)) {
      return _webhookSecrets[source];
    }
    ...
    final rows = await db.query(
      'app_config',
      where: 'clave = ?',
      whereArgs: ['webhook_secret_${source.name}'],
      limit: 1,
    );

    if (rows.isEmpty) return null;
    final secret = rows.first['valor']?.toString();
    if (secret != null) {
      _webhookSecrets[source] = secret;
    }
    return secret;
  ...
  bool validarFirmaHmac(
    ...
    return _constantTimeEquals(digest.toString(), signature);
  }
