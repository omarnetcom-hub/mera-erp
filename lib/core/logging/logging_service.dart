...
class LoggingService {
  ...
  void warning(String message, {String? module, Map<String, dynamic>? metadata}) {
    _log(LogLevel.warning, message, module: module, metadata: metadata);
  }
  
  /// Registra un log de nivel ERROR
  void error(String message, {String? module, String? stackTrace, Map<String, dynamic>? metadata}) {
    _log(LogLevel.error, message, module: module, stackTrace: stackTrace, metadata: metadata);
  }
  
  /// Registra un log de nivel CRITICAL
  void critical(String message, {String? module, String? stackTrace, Map<String, dynamic>? metadata}) {
    _log(LogLevel.critical, message, module: module, stackTrace: stackTrace, metadata: metadata);
  }
  ...
  /// Maneja logs críticos
  void _handleCriticalLog(LogEntry entry) {
    // TODO: Implementar alertas (email, notificación push, etc.)
  ...
  Future<File> exportLogsToFile({String? filename}) async {
    final directory = await getApplicationDocumentsDirectory();
    final file = File('${directory.path}/${filename ?? 'logs_export_${DateTime.now().millisecondsSinceEpoch}.json'}');
