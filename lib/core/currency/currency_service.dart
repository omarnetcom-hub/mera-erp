...
class CurrencyService {
  ...
  /// Actualiza tasas de cambio desde API externa
  Future<bool> updateRatesFromAPI(Database db, int companyId, String baseCurrency) async {
    try {
      // Usar API gratuita de tasas de cambio (ej: exchangerate-api.com)
      ...
      if (response.statusCode == 200) {
        ...
        // Actualizar tasas para cada moneda disponible
        ...
        for (final currency in currencies) {
          ...
          if (rate != null) {
            final exchangeRate = ExchangeRate(
              ...
              effectiveDate: date,
              expiryDate: date.add(const Duration(days: 1)), // Vence en 1 día
              source: 'api',
              createdAt: date,
            );
            
            await registerExchangeRate(db, exchangeRate);
          }
        }
  ...
  Future<Map<String, dynamic>?> getCompanyCurrencyConfig(Database db, int companyId) async {
    final configResult = await db.query(
      'company_settings',
      where: 'company_id = ? AND clave = ?',
    ...
    return {
      'base_currency': configResult.first['valor'],
      'default_currency': await getDefaultCurrency(db),
    };
  }
  
  /// Establece la moneda base de una empresa
  Future<void> setCompanyBaseCurrency(Database db, int companyId, String currencyCode) async {
    await db.insert(
      'company_settings',
      {
  ...
  Future<String> formatAmount(Database db, double amount, String currencyCode) async {
    final currencies = await getAllCurrencies(db);
