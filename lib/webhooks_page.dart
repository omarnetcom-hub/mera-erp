...
class _WebhooksPageState extends State<WebhooksPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final webhooks = await _webhookService.getWebhooksByCompany(db, companyId);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error al cargar webhooks: $e')),
        );
      }
    }
  }
  ...
  Future<void> _toggleWebhookActive(Webhook webhook) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(webhook.isActive ? 'Webhook desactivado' : 'Webhook activado')),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _deleteWebhook(Webhook webhook) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('¿Está seguro de eliminar el webhook para el evento "${webhook.event}"?'),
        actions: [
          TextButton(
    ...
    if (confirmed == true) {
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Webhook eliminado')),
          );
        }
      } catch (e) {
        if (mounted) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        leading: CircleAvatar(
                          backgroundColor: webhook.isActive ? Colors.green : Colors.grey,
                          child: Icon(
                        ...
                        trailing: _isUpdating
                            ...
                            : PopupMenuButton<String>(
                                ...
                                itemBuilder: (context) => [
                                  PopupMenuItem(
                                    value: 'toggle',
                                    child: Text(webhook.isActive ? 'Desactivar' : 'Activar'),
                                  ),
                                  PopupMenuItem(
                                    value: 'delete',
