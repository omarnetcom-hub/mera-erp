...
| Cierre de vigencia y vigencias futuras | `contabilidad/services/cierre_vigencia_service.dart`; `presupuesto_service.dart` | Ninguno identificado | Inspeccion ejecutada en sesion autonoma: `_calcularReservas` solo suma saldos clase 24 y provisiones; no consulta `obligaciones` ni existe registro de recibido sin obligacion. No se encontro implementacion de vigencias futuras, Confis o MFMP. | Parcial - requiere modelar reservas presupuestales y cuentas por pagar de recibidos sin obligacion; vigencias futuras requiere diseno humano y autorizacion plurianual. |
