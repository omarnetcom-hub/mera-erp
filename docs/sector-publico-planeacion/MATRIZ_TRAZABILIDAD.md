...
| NICSP 1: estados financieros | `contabilidad/services/cierre_vigencia_service.dart`; `contabilidad/services/contabilidad_nicsp_service.dart`; `models/estado_financiero.dart`; `pages/contabilidad_nicsp_page.dart` | Ninguno ejecutado | Inspeccion ejecutada: los generadores usan `saldos_cuentas`, alimentados por asientos; pero suman pasivo/patrimonio con signo acreedor negativo y no integran el resultado del periodo al patrimonio, por lo que un estado derivado de asientos normales no cuadra. | Parcial - existe generacion, pero requiere correccion y prueba de estados cuadrados antes de certificarse. |
...
| Fase contractual: contrato, polizas y ejecucion | `models/contrato.dart`; `models/poliza.dart`; `contratacion_service.dart` | `contratacion_service_test.dart` (inspeccion) | Inspeccion de diseno en `SECOP_VALIDACIONES_LOCALES_BLOQUEO_DISENO.md`: la UI crea contrato seleccionando RP, pero el RP debe referir contrato firmado y no existe transicion a `firmado`. | Parcial - requiere decidir e implementar el flujo contrato firmado -> RP -> polizas -> legalizacion; no se certificaron polizas ni controles de ejecucion. |
...
| CHIP: CGN2015_001 a 005 y CGN2016C01 | `auditoria/models/reporte_chip.dart`; `auditoria/services/chip_reporter_service.dart`; `auditoria/pages/auditoria_forense_page.dart` | Ninguno ejecutado | Inspeccion ejecutada: cada generador recibe DTOs manuales desde la UI; no consulta datos contables/presupuestales. Los nombres/contenidos del modelo no corresponden 1:1 con la tabla CHIP del plan v1.1. | Parcial - falta mapeo desde fuentes reales, alineacion de formularios CGN y prueba contra estructura oficial. |
